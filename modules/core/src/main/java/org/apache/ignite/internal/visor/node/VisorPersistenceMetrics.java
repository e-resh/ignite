...
public class VisorPersistenceMetrics extends VisorDataTransferObject {
     ...
     */
    public VisorPersistenceMetrics(GridMetricManager gmm) {

        ReadOnlyMetricRegistry mreg = gmm.registry(DATASTORAGE_METRIC_PREFIX);

        walLoggingRate = ((float)mreg.<HitRateMetric>findMetric("WalLoggingRate").value() * 1000 / mreg.<HitRateMetric>findMetric("WalLoggingRate").rateTimeInterval());
        walWritingRate = ((float)mreg.<HitRateMetric>findMetric("WalWritingRate").value() * 1000 / mreg.<HitRateMetric>findMetric("WalWritingRate").rateTimeInterval());
        walArchiveSegments = mreg.<IntMetric>findMetric("WalArchiveSegments").value();
        walFsyncTimeAvg = (float)mreg.<DoubleMetric>findMetric("walFsyncTimeAverage").value();
        walBufPollSpinRate = mreg.<LongMetric>findMetric("WalBuffPollSpinsRate").value();
        walSz = mreg.<LongMetric>findMetric("WalTotalSize").value();
        walLastRollOverTm = mreg.<LongMetric>findMetric("WalLastRollOverTime").value();

        cpTotalTm = mreg.<LongMetric>findMetric("CheckpointTotalTime").value();

        lastCpDuration = mreg.<LongMetric>findMetric("LastCheckpointDuration").value();
        lastCpStart = mreg.<LongMetric>findMetric("LastCheckpointStart").value();
        lastCpLockWaitDuration = mreg.<LongMetric>findMetric("LastCheckpointLockWaitDuration").value();
        lastCpMmarkDuration = mreg.<LongMetric>findMetric("LastCheckpointMarkDuration").value();
        lastCpPagesWriteDuration = mreg.<LongMetric>findMetric("LastCheckpointPagesWriteDuration").value();
        lastCpFsyncDuration = mreg.<LongMetric>findMetric("LastCheckpointFsyncDuration").value();
        lastCpTotalPages = mreg.<LongMetric>findMetric("LastCheckpointTotalPagesNumber").value();
        lastCpDataPages = mreg.<LongMetric>findMetric("LastCheckpointDataPagesNumber").value();
        lastCpCowPages = mreg.<LongMetric>findMetric("LastCheckpointCopiedOnWritePagesNumber").value();

        dirtyPages =  mreg.<LongMetric>findMetric("DirtyPages").value();
        pagesRead = mreg.<LongMetric>findMetric("PagesRead").value();
        pagesWritten = mreg.<LongMetric>findMetric("PagesWritten").value();
        pagesReplaced = mreg.<LongMetric>findMetric("PagesReplaced").value();

        offHeapSz = mreg.<LongMetric>findMetric("OffHeapSize").value();
        offheapUsedSz = mreg.<LongMetric>findMetric("OffheapUsedSize").value();

        mreg.findMetric("UsedCheckpointBufferPages");
        mreg.findMetric("UsedCheckpointBufferSize");
        mreg.findMetric("UsedCheckpointBufferSize");

        usedCpBufPages = mreg.<LongMetric>findMetric("UsedCheckpointBufferPages").value();
        usedCpBufSz = mreg.<LongMetric>findMetric("UsedCheckpointBufferSize").value();
        cpBufSz = mreg.<LongMetric>findMetric("UsedCheckpointBufferSize").value();

        totalSz =  mreg.<LongMetric>findMetric("TotalAllocatedSize").value();

        mreg.findMetric("StorageSize");
        mreg.findMetric("SparseStorageSize");
